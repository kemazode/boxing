...
int box_exist(struct box_parser *bp, const char *box);
...
int main(int argc, char **argv)
{
	...
	if (args_info.create_given) box_create(parser, args_info.box_arg);
	if (args_info.append_given) box_append(parser, args_info.box_arg, args_info.append_arg); 
	if (args_info.read_given) box_read(parser, args_info.box_arg);
	if (args_info.delete_given) box_remove(parser, args_info.box_arg);
	if (args_info.list_given) box_list(parser);
	
	if (REWRITE) box_rewrite(parser, args_info.inputs[0]);
...
int box_append(struct box_parser *bp, const char *box, char *items)
{
	if (!box_exist(bp, box)) box_create(bp, box);
...
int box_remove(struct box_parser *bp, const char *box)
{
	...
	while (token = box_parser_token_next(bp)) 
		if (token->type == BOX_TITLE && !strcmp(token->value, box)) {		 
			while ((token = box_parser_token_remove(bp))
				&& token->type == BOX_ITEM);
			box_parser_rewind(bp);
			break;
		}
...
{
	if (box_exist(bp, box)) {
		fprintf(stderr, "%s: box already exist\n", box);
		return false;
	}
...
int box_list(struct box_parser *bp)
{
	const struct box_token *token = NULL;
	while (token = box_parser_token_next(bp)) 	   
		if (token->type == BOX_TITLE) printf("%s\n", token->value);
	box_parser_rewind(bp);
}
